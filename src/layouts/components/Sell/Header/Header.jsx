...
// xóa trùng
  ...
  const handleSearch = () => {
    if (keyword.trim()) {
      ...
      navigate(
        `/ProductSell?search=${encodeURIComponent(keyword.trim())}`
      );
      ...
      <div className={cx('container')}>
        <div className={cx('headerTop')}>
          <div className={cx('infoHeader')}>
            <FontAwesomeIcon icon={faFacebook} className={cx('iconFace')} />
            <p>For any queries - please contact customer care-0367376403</p>
          </div>
        </div>
                    ...
                    <li key={index} className={cx('historyItem')}>
                      <span
                        onClick={() => {
                          ...
                          setShowHistory(false);
                          navigate(
                            `/ProductSell?search=${encodeURIComponent(item)}`
                          );
                        }}
                      >
                        {item}
            ...
            <div className={cx('menuIconHeader')}>
              <ul>
                <li>
                  <Link
                    to="/NotificationSell"
                    className={cx({ active: isActive('/NotificationSell') })}
                  >
                ...
                <li>
                  <Link
                    to="/MessageSell"
                    className={cx({ active: isActive('/MessageSell') })}
                  >
                ...
                <li>
                  <Link
                    to="/ProfileSell"
                    className={cx({ active: isActive('/ProfileSell') })}
                  >
