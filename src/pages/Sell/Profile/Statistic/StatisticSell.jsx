...
function StatisticSell() {
    ...
    const handleFilterChange = (valueYear, valueMonth, valueWeek) => {
        ...
        /**
         * Ở đây bạn khởi tạo URLSearchParams() trống hoàn toàn.
            Ban đầu newParams không có tham số nào
         */
        if (valueYear) {
    ...
    const chartData = useMemo(() => {
        ...
        if (calendarData.days?.length > 0) { // nếu nó tồn tai thì trả về calendarData.days?.length
    ...
    const sumorder = infoOrder.reduce(
        (acc, item) =>
            acc + (item.product.price - item.discount + item.shippingFee),
        0,
    );
            ...
            <div className={cx('contentHeader')}>
                ...
                <div className={cx('selectitem')}>
                    <Select
                        ...
                        value={Year}
                        onChange={(value) =>
                            handleFilterChange(value, Month, Week)
                        }
                    ...
                    <Select
                        ...
                        value={Month}
                        onChange={(value) =>
                            handleFilterChange(Year, value, Week)
                        }
                    />
                    <Select
                        data={listSelect[5]}
                        value={Week}
                        onChange={(value) =>
                            handleFilterChange(Year, Month, value)
                        }
                    />
                </div>
            </div>
