...
import { storeList, listSelect } from '~/constant/mock-data';
...
// lắng nghe event clear search để xóa đi từ khóa tìm kiếm
  useEffect(() => {
    ...
    const filteredOrders = storeList.filter((order) => {
        if (filters.status === 'Tất Cả') return true;
        return order.roll === filters.status;
    });
    const filteredOrders2 = storeList.filter((order) => {
        ...
        <div className={cx('container')}>
            ...
            <div className={cx('formSearch')}>
                ...
                <div className={cx('content')}> 
                       <ItemManager data={'store'} status={filters.status} area={filters.area} />
                </div>
